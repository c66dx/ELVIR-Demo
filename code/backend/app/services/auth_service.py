...
from app.core.password_policy import activation_password_error_code, raise_if_password_too_short
        ...
        if youth and youth.user_id:
            ...
            if user:
                if user.is_active:
                    ...
                    err = activation_password_error_code(data.password)
                    if err:
                        return ActivateResponse(success=False, error=err)
...
def change_password(db: Session, user: User, data: ChangePasswordRequest) -> dict:
    raise_if_password_too_short(data.new_password, field_label="La nueva contraseña")
    if not verify_password(data.current_password, user.password_hash):
