...
class Settings(BaseSettings):
    ...
    # Longitud mínima de contraseña (activación, cambio de contraseña). Default 6 en dev.
    # En ENV=prod el validador exige al menos 12 (o mayor si configuras un valor más alto).
    ...
    def validate_production_settings(self):
        ...
        # Evita que espacios o CRLF en .env dejen IDs vacíos o inválidos al hacer merge con plantilla.
        self.LIVEAVATAR_API_KEY = (self.LIVEAVATAR_API_KEY or "").strip()
        self.LIVEAVATAR_CONTEXT_ID = (self.LIVEAVATAR_CONTEXT_ID or "").strip()
        ...
        min_pw = int(self.PASSWORD_MIN_LENGTH)
        if min_pw < 6:
            raise ValueError("PASSWORD_MIN_LENGTH must be at least 6")
        if self.is_production and min_pw < 12:
            raise ValueError("PASSWORD_MIN_LENGTH must be at least 12 in production (set env PASSWORD_MIN_LENGTH)")
        self.PASSWORD_MIN_LENGTH = min_pw
