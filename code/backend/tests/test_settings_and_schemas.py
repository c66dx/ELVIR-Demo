...
_PROD_PW_MIN = 12  # must match production PASSWORD_MIN_LENGTH floor in Settings validator
...
class SettingsAndSchemasTestCase(unittest.TestCase):
    ...
    def test_production_alias_is_accepted_when_safe(self):
        s = Settings(
            ...
            PASSWORD_MIN_LENGTH=_PROD_PW_MIN,
        )
    ...
    def test_production_rejects_default_secret(self):
        with self.assertRaises(ValidationError):
            Settings(
                ENV="prod",
                SECRET_KEY="elvir-dev-secret-change-in-production",
                AUTO_CREATE_TABLES=False,
                CORS_ORIGINS="http://localhost:4200",
                PASSWORD_MIN_LENGTH=_PROD_PW_MIN,
            )
    ...
    def test_production_rejects_password_min_below_12(self):
        with self.assertRaises(ValidationError):
            Settings(
                ENV="prod",
                SECRET_KEY=_PROD_SECRET,
