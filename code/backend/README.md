...
Para una visión orientada a **revisión técnica o auditoría** (capas, seguridad, CI), ver [ARCHITECTURE.md](ARCHITECTURE.md). Listado de rutas y convenciones HTTP (**v1.3**): [`../../docs/api/endpoints.md`](../../docs/api/endpoints.md).

## Requisitos

- **Python 3.11 o superior** (obligatorio). Con **3.10 o anterior** el proyecto no arranca: se usa sintaxis y APIs de la biblioteca estándar de 3.11 (`enum.StrEnum`, `datetime.UTC`, etc.). El **CI** y la **imagen Docker** usan 3.11.
- pip (o un entorno virtual recomendado)

Comprueba la versión: `python --version` (debe ser 3.11.x).

Si el servicio de despliegue usa **solo** esta carpeta como raíz (p. ej. algunos PaaS), el archivo `runtime.txt` junto a este README ayuda a fijar Python 3.11. Si la raíz del repo es la del monorepo, usa el `runtime.txt` de la raíz del proyecto.
...
- `PASSWORD_MIN_LENGTH`: Longitud mínima de contraseña en activación de cuenta y cambio de contraseña (default **6** en desarrollo). En **`ENV=prod`** debe ser **≥ 12** (validado al arrancar). Alinear el build del frontend (`environment.prod.ts` → `passwordMinLength`) con este valor.
