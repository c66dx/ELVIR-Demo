...
import {
  ...
  type YouthNotification,
} from '../../../core/services/youth-notifications.service';

@Component({
  selector: 'app-notifications-joven',
  standalone: true,
  imports: [AsyncPipe, RouterLink],
  templateUrl: './notifications-joven.component.html',
  styleUrl: './notifications-joven.component.scss',
})
export class NotificationsJovenComponent {
  ...
  data$ = combineLatest([this.youthService.getCurrentYouthId(), this.page$, this.refresh$]).pipe(
    switchMap(([youthId, page]) =>
      youthId
        ? this.notifications.getYouthNotifications(youthId, { page, page_size: this.pageSize })
        : of({ items: [], total: 0, unread: 0, page: 1, page_size: this.pageSize })
    )
  );
