...
@Component({
  selector: 'app-retroalimentacion-detail-joven',
  standalone: true,
  imports: [CommonModule, RouterLink],
  templateUrl: './retroalimentacion-detail.component.html',
  styleUrl: './retroalimentacion-detail.component.scss',
})
export class RetroalimentacionDetailJovenComponent {
  ...
  vm$ = this.route.paramMap.pipe(
    ...
    switchMap((sessionId) => {
      ...
      return forkJoin({
        session: this.api.getSession(sessionId).pipe(catchError(() => of(null))),
        context: this.api.getSessionContext(sessionId).pipe(catchError(() => of(null))),
        summary: this.api.getSessionSummary(sessionId).pipe(catchError(() => of(null))),
      }).pipe(
        map(({ session, context, summary }) => {
          ...
          const detail: FeedbackDetail = {
            ...
            jobRoleName: context?.jobRoleName,
