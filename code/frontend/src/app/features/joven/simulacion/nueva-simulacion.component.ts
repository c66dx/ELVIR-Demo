...
import { NuevaSimulacionFacade } from '@features/joven/simulacion/nueva-simulacion.facade';
...
export class NuevaSimulacionComponent implements OnInit {
  ...
  private facade = inject(NuevaSimulacionFacade);
  ...
  private readonly companyContexts: Record<string, { name: string; description: string }> = {
    operario: {
      ...
      description: 'una empresa dedicada a la distribución y apoyo en bodegas.',
    },
    atencion_publico: {
      name: 'Centro Ciudadano',
      description: 'una organización que orienta a personas en servicios y trámites.',
    },
    administrativo: {
      name: 'Clínica Horizonte',
      description: 'un centro de salud que coordina agenda y documentación interna.',
    },
    tecnico_profesional: {
      name: 'TecnoSoluciones',
      description: 'una empresa que implementa soporte y servicios técnicos a clientes.',
    },
  };
  ...
  ngOnInit(): void {
    ...
    this.facade.loadCatalogs().subscribe({
      next: ({ jobRoles, cases }) => {
  ...
  getCompanyContext(role: JobRole | null): { name: string; description: string } {
    ...
    return (
      this.companyContexts[role.slug] ?? {
        name: 'una empresa',
        description: 'una organización del sector laboral.',
      }
    );
  }
  ...
  isDynamicCase(item: Case): boolean {
    ...
    this.youthService
      ...
      .pipe(
        switchMap((youthId) => {
          ...
          return this.facade.getSimulationTemplates({ job_role_id, case_id }).pipe(
            switchMap((templates) => {
              ...
              return this.facade.createSession({
                youth_id: youthId,
