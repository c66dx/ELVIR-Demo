...
import type { SimulationTemplate } from '../../../core/models/simulation-template.model'; 
 /**   * Formulario para elegir cargo y caso. Crea sesión AUTOGESTIONADA y navega a la simulación.   * Usa la primera plantilla que coincida con la combinación seleccionada.   */
@Component({ 
 selector: 'app-nueva-simulacion',   standalone: true,   imports: [ReactiveFormsModule],   templateUrl: './nueva-simulacion.component.html',   styleUrl: './nueva-simulacion.component.scss',
})
export class NuevaSimulacionComponent implements OnInit { 
 ...
 switch (item.difficulty) { 
 case 'BAJA':   return 'La entrevistadora ser cercana y dar tiempo para responder.'; 
 ...
 this.youthService   .getCurrentYouthId()   .pipe(   switchMap((youthId) => { 
 if (!youthId) { 
 ...
 youth_id: youthId,   simulation_template_id: template.id,   mode: 'AUTOGESTIONADA',   }); 
 })   ); 
 ...
 state: { 
 companyName: company.name,   companyDescription: company.description,   jobRoleName: role?.name ?? '',   caseName: caseItem?.name  ??  '',   },   }); 
 } 
 },   error: () => { 
