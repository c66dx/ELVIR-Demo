...
export class DashboardJovenComponent { 
 ...
 return forkJoin({ 
 sessionsPage: this.api.getSessionsPaged({ youth_id: youthId, page: 1, page_size: 5 }),   stats: this.api.getSessionStats({ youth_id: youthId, months: 6 }),   suggestionsMeta: this.api.getYouthMaterialSuggestionsPaged(youthId, { page: 1, page_size: 1 }),   }).pipe(   map(({ sessionsPage, stats, suggestionsMeta }) => { 
