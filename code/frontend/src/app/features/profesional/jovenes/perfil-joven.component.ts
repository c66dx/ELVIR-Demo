...
import { PerfilJovenFacade } from '@features/profesional/jovenes/perfil-joven.facade';
 ...
 id: 'comunicacion',   title: 'Comunicación',   description: 'Claridad y estructura al expresar ideas durante la entrevista.',   unit: '%',   x: ['Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul'],   series: [{ name: 'Puntaje', data: [48, 55, 61, 66, 70, 74] }],   },   { 
...
export class PerfilJovenComponent implements OnInit { 
 ...
 material_id: ['', Validators.required],   reason: [''],   session_id: [''],   }); 
 } 
...
// Scroll a la sección después de que Angular la renderice: setTimeout(() => this.scrollToSuggestMaterial(), 100);
 ...
 /** Métricas de desempeño: total, completadas, tasa */   metricsSummary = signal<{ total: number; completed: number; cancelled: number; error: number; completionRate: number } | null>(null); 
 readonly checklistItems = PROFILE_CHECKLIST_ITEMS; 
 getChecklistLabels(youth: Youth): string[] { 
 const slugs = youth.profile_checklist  ?? []; 
 return slugs   .map((s) => this.checklistItems.find((i) => i.slug === s)?.label)   .filter((l): l is string => !!l); 
 } 
 ...
 competency_slug: c.slug,   level_slug: levelBySlug.get(c.slug)  ??  null,   })); 
 ...
 const updated = current.map((row) =>   row.competency_slug === competencySlug ?  { ...row, level_slug: levelSlug || null } : row   ); 
