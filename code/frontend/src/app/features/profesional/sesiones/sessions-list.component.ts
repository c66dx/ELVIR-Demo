...
import { formatDate, formatDuration } from '../../../shared/utils/date-format.util'; 
 ...
 @Component({ 
 selector: 'app-sessions-list',   standalone: true,   imports: [RouterLink, StatusBadgeComponent, FormsModule],   templateUrl: './sessions-list.component.html',   styleUrl: './sessions-list.component.scss',
})
export class SessionsListComponent implements OnInit { 
 ...
 readonly statusOptions: { value: 'EN_CURSO' | 'COMPLETADA' | 'CANCELADA' | 'ERROR'; label: string }[] = [   { value: 'EN_CURSO', label: 'En curso' },   { value: 'COMPLETADA', label: 'Completada' },   { value: 'CANCELADA', label: 'Cancelada' },   { value: 'ERROR', label: 'Error' },   ]; 
 readonly modeOptions: { value: 'AUTOGESTIONADA' | 'SUPERVISADA'; label: string }[] = [   { value: 'AUTOGESTIONADA', label: 'Autogestionada' },   { value: 'SUPERVISADA', label: 'Supervisada' },   ]; 
 ngOnInit(): void { 
 ...
 id: y.id,   display_name: y.display_name,   rut: y.rut,   })); 
 },   }); 
 } 
 loadSessions(): void { 
 ...
 page: this.page,   page_size: this.pageSize,   youth_id: this.filterYouthId || undefined,   search: this.filterSearch || undefined,   status: this.filterStatus || undefined,   mode: this.filterMode || undefined,   start_date: this.filterStartDate || undefined,   end_date: this.filterEndDate || undefined,   })   .pipe(   switchMap((paged) => { 
 ...
 ...s,   youthName: youthMap.get(s.youth_id)?.display_name ?? 'Desconocido',   youthRut: youthMap.get(s.youth_id)?.rut,   youthPhotoUrl: youthMap.get(s.youth_id)?.profile_photo_url,   })); 
