...
import { SelectInputComponent } from '../../../shared/form/inputs/select-input/select-input.component'; 
 @Component({ 
 selector: 'app-material-form',   standalone: true,   imports: [   AsyncPipe,   ReactiveFormsModule,   RouterLink,   FormContainerComponent,   FormSectionComponent,   FormGridComponent,   FormFieldComponent,   FormActionsComponent,   TextInputComponent,   TextareaInputComponent,   SelectInputComponent,   ],   templateUrl: './material-form.component.html',   styleUrl: './material-form.component.scss',
})
export class MaterialFormComponent { 
 ...
 readonly types: { value: MaterialType; label: string }[] = [   { value: 'VIDEO', label: 'Video' },   { value: 'PDF', label: 'PDF' },   { value: 'LINK', label: 'Enlace' },   ]; 
 form: FormGroup = this.fb.nonNullable.group({ 
 title: ['', Validators.required],   description: [''],   type: ['VIDEO' as MaterialType, Validators.required],   url: [''],   job_role_id: [''],   case_id: [''],   }); 
 constructor() { 
 ...
 submitting = false; 
 errorMessage = ''; 
 /** Archivo seleccionado para subir (VIDEO o PDF). */   selectedFile: File | null = null; 
 ...
 title: v.title,   description: v.description || undefined,   type: v.type,   url,   job_role_id: v.job_role_id || undefined,   case_id: v.case_id || undefined,   }).subscribe({ 
 next: (result) => { 
