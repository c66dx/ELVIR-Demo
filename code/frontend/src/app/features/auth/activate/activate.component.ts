...
export class ActivateComponent implements OnInit { 
 ...
 /** Longitud mínima de contraseña (alineada con backend / environment). */
 readonly passwordMinLength = environment.passwordMinLength;
 ngOnInit(): void { 
 ...
 passwordCtrl.setValidators([Validators.required, Validators.minLength(this.passwordMinLength)]); 
 ...
 if (password?.trim() && password.length < this.passwordMinLength) {
 this.form.get('password')?.setErrors({ minlength: { requiredLength: this.passwordMinLength } });
 return;
 } 
 } else { 
