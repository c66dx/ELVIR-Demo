...
const REQUEST_ID_HEADER = 'X-Request-ID'; 
 ...
 /**   * Interceptor HTTP que:   * - Envía cookies de sesión (`withCredentials`) para auth con HttpOnly cookie.   * - Adjunta header CSRF para métodos mutables.   * - Mantiene compatibilidad opcional con Bearer token en memoria.   * - Ante 401, hace logout y redirige a /login (excepto en login y activación).   */
export const authInterceptor: HttpInterceptorFn = (req, next) => { 
 ...
 withCredentials: false,   setHeaders: headers,   }); 
