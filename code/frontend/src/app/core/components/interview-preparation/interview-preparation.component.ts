...
export class InterviewPreparationComponent implements OnInit {
  ...
  caseName = '';
  contextSummary = 'Esta entrevista simula un entorno laboral real y se enfocará en tus habilidades profesionales.';
  ...
  startInterview(): void {
    ...
    const target =
      this.route.snapshot.data?.['target'] === 'profesional'
        ? '/profesional/simulacion'
        : '/joven/simulacion';
