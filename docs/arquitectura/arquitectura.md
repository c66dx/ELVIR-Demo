...
- **Integración con LiveAvatar (Context Dinámico):** antes de cada sesión, el backend obtiene el prompt correspondiente (contenido de diseño; material de referencia opcional si existe en el entorno). Detalle técnico: `docs/liveavatar/integracion-liveavatar.md`. Luego hace PATCH al contexto en LiveAvatar, crea la sesión y devuelve token/URL al frontend.
...
- El backend de ELVIR obtiene el prompt (contenido de diseño / catálogo) y lo envía a LiveAvatar antes de cada sesión (Context Dinámico).
...
3. El backend:
   ...
   - Obtiene el prompt correspondiente (archivos de Prompt Builder o catálogo en BD; contenido de diseño).
...
**Observabilidad:** el API expone cabecera `X-Request-ID` en las respuestas, endpoints de **salud** en la raíz del servicio (`/health`, `/health/ready`, `/health/metrics`, `/health/live`, exentos de rate limit para probes) y, en entornos configurados, **rate limiting** por IP en rutas sensibles. Detalle de rutas: `docs/api/endpoints.md` y `docs/errors.md`.
...
- Implementar o entrenar modelos de IA conversacional propios.
- Análisis avanzado de desempeño y métricas complejas más allá de lo que ya registra la API.
- Funcionalidades propias de un producto productivo (facturación, multi-tenant, etc.).

> Nota: el MVP **sí** persiste metadatos de audio de sesión y acepta evaluaciones externas vía webhook cuando están configuradas; no obstante, la conversación y el avatar siguen siendo responsabilidad del proveedor externo. Si LiveAvatar entrega resúmenes o evaluaciones, ELVIR puede **almacenarlas y mostrarlas** sin implementar esos modelos internamente.
