# API REST – Endpoints del backend ELVIR (v1.3, opción A)
...
**v1.3:** documenta estadísticas de sesión, contexto UI, heartbeat, webhook de evaluación, audio de sesión, healthchecks y corrige el alcance (funciones ya implementadas).
...
### 1.1 Salud y entrada (sin prefijo `/api/v1`)

Estas rutas están en la raíz del servicio (no bajo `/api/v1`). Están **exentas** del rate limit habitual (útil para probes y monitores).

| Método | Ruta | Descripción |
|--------|------|-------------|
| `GET` | `/` | Mensaje raíz (`ELVIR API`) y enlace a documentación OpenAPI (`/docs`). |
| `GET` | `/health` | Resumen rápido: servicio arriba (sin comprobar BD). |
| `GET` | `/health/metrics` | Métricas operativas (requests, tasa 5xx) y alertas simples (`error_rate_high`). |
| `GET` | `/health/live` | Liveness: el proceso responde (sin BD). |
| `GET` | `/health/ready` | Readiness: comprueba BD (`SELECT 1`). Respuesta **503** si la base no es accesible. |
...
### 7.3 Estadísticas agregadas

**GET** `/sessions/stats`

Resumen de sesiones visibles para el usuario autenticado: conteos por estado y serie mensual de completadas.

**Query (opcionales):** `youth_id` (≥1), `months` (1–24, por defecto 6).
...
### 7.5 Etiquetas de contexto (UI)

**GET** `/sessions/{session_id}/context`

Devuelve nombres legibles de cargo y caso asociados a la plantilla de la sesión (p. ej. `jobRoleName`, `caseName`) para mostrar en la pantalla de simulación. Requiere acceso a la sesión.
...
Actualiza `last_heartbeat_at` en una sesión en curso (evitar cierre por inactividad). Requiere acceso a la sesión.

---

### 7.7 Orquestación: iniciar sesión en LiveAvatar

**POST** `/sessions/{session_id}/start`

Inicia la experiencia con LiveAvatar según la plantilla. El backend obtiene el prompt (contenido de diseño; archivos o catálogo), hace PATCH al contexto en LiveAvatar, crea la sesión y devuelve los datos para que el frontend conecte (LiveKit: `livekit_url`, `access_token`; o embed si aplica). Ver `docs/liveavatar/integracion-liveavatar.md`.
...
### 7.9 Webhook de evaluación externa

**POST** `/sessions/evaluation`

Recibe una evaluación desde un servicio externo (p. ej. LiveAvatar) y la fusiona en `sessions.metrics`. **No usa** `Authorization: Bearer`; si en el backend está configurado `LIVEAVATAR_WEBHOOK_SECRET`, el cliente debe enviar el header `X-ELVIR-Webhook-Secret` con ese valor (si no, el endpoint responde 401).
...
{
  "session_id": 42,
  "evaluation": { "score": 0.85, "notes": "..." },
  "source": "liveavatar"
}
~~~

Alternativa: identificar la sesión con `liveavatar_session_id` en lugar de `session_id`.

#### Response (ejemplo)

~~~json
{ "ok": true, "session_id": 42 }
~~~

---

### 7.10 Obtener transcripción de sesión
...
### 7.11 Audio de sesión

**POST** `/sessions/{session_id}/audio`

Sube el archivo de audio grabado de la sesión (`multipart/form-data`: campo `file` obligatorio; `duration_seconds` opcional). Joven o profesional con acceso a la sesión.

**GET** `/sessions/{session_id}/audio`

Devuelve metadatos del audio (URL, tipo, tamaño, duración) o `null` si no hay registro. Requiere acceso a la sesión.

#### Response GET (ejemplo)

~~~json
{
  "id": 7,
  "session_id": 42,
  "url": "https://api.example/uploads/session-audio/…",
  "content_type": "audio/webm",
  "file_size_bytes": 245000,
  "duration_seconds": 120,
  "created_at": "2026-02-10T15:50:00Z",
  "updated_at": "2026-02-10T15:50:00Z"
}
~~~
...
La API definida en este documento cubre el alcance funcional del MVP: autenticación, jóvenes y asignaciones, catálogos, sesiones (incl. estadísticas, eventos, transcripción, **audio de sesión**, evaluación por competencias, webhook de evaluación externa), resúmenes y material de apoyo.

**Fuera de alcance o diferido** (no forman parte del núcleo entregado aquí):

- Entrenar o alojar modelos de IA conversacional propios (el avatar es servicio externo).
- Análisis avanzado de desempeño más allá de métricas básicas y datos que ya persisten el sistema.
- Streaming en tiempo real o integraciones de terceros no cubiertas por el contrato actual con LiveAvatar.

Extensiones futuras pueden añadirse sin romper la estructura base (`/api/v1`, esquemas y roles).
