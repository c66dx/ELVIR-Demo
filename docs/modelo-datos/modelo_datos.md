...
**Integración Context Dinámico:** los campos opcionales `description`, `objetivo` y `competencias` permiten construir el prompt dinámico para LiveAvatar (ver `docs/liveavatar/integracion-liveavatar.md`).
...
### 5.5 SESSION_AUDIO (`session_audios`)

Almacena **metadatos** del audio grabado de una sesión (URL del archivo, tipo MIME, tamaño, duración). La subida se hace por API (`POST /sessions/{id}/audio`); el binario no vive en la tabla (solo referencia).

- Relación 1:1 con SESSIONS (como mucho un registro de audio por sesión).
- Complementa la transcripción textual de `SESSION_TRANSCRIPTS` cuando se requiere evidencia de audio.
