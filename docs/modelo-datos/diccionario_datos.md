...
## 11.5 SESSION_AUDIOS (`session_audios`)

**Propósito:** metadatos del archivo de audio asociado a una sesión (grabación subida por cliente).  
**Regla:** 0..1 fila por sesión (`session_id` único).

| Campo | Tipo | Restricciones | Descripción |
|------|------|---------------|-------------|
| id | int | PK, NN | Identificador |
| session_id | int | FK → SESSIONS.id, NN, UQ | Sesión |
| url | string | NN | URL o ruta servida al cliente |
| content_type | string | nullable | MIME (ej. `audio/webm`) |
| file_size_bytes | int | nullable | Tamaño |
| duration_seconds | int | nullable | Duración declarada |
| created_at | datetime | NN | Creación |
| updated_at | datetime | NN | Última actualización |
...
- SESSIONS (con status + timestamps + liveavatar_session_id); opcionalmente SESSION_TRANSCRIPTS, SESSION_AUDIOS, SESSION_EVENTS
